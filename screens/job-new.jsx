...
function JobNew({ nav, session, setLiveJobs }) {
  const [client, setClient] = useStateN(CLIENTS[1]);
  const [format, setFormat] = useStateN('20s');
  const [template, setTemplate] = useStateN(TEMPLATES[0]);
  const [promptUx, setPromptUx] = useStateN('structured'); // structured | freeform | shotlist
  const [freeform, setFreeform] = useStateN("James walks through a modern staged living room, stops, turns to camera and says \"This 3-bed in Smithfield just hit market — and it won't last.\" Golden hour light, dolly in. @Image1 @Image2 @Image3 @Audio1 as references.");
  const [selectedImgs, setSelectedImgs] = useStateN([1,2,3,4]);
  const [submitting, setSubmitting] = useStateN(false);
  const [err, setErr] = useStateN("");

  const nClips = format === '20s' ? 2 : 3;
  const estCost = (nClips * 3.0).toFixed(2);

  const submitJob = async () => {
    setErr("");
    setSubmitting(true);
    try {
      const job = await VT_API.createJob(session.access_token, {
        client_slug: client.slug,
        format_seconds: Number(format.replace("s","")),
        idempotency_key: `ui-${Date.now()}`,
      });
      setLiveJobs(prev => ({...prev, [job.id]: job}));
      nav('clip-approve', job.id);
    } catch (e) {
  ...
  return <div style={{padding:'24px 28px', maxWidth:1480, margin:'0 auto'}}>
    ...
    <div style={{display:'flex', alignItems:'center', gap:12, marginBottom:20}}>
      ...
      <div style={{marginLeft:'auto', display:'flex', gap:10, alignItems:'center'}}>
        ...
        <Btn variant="primary" icon="bolt" onClick={submitJob} disabled={submitting}>{submitting ? "Submitting..." : "Generate clip 1"}</Btn>
      </div>
    </div>
    {err && <div style={{marginBottom:12, color:"#F06571", fontSize:12}}>{err}</div>}
      ...
      <div style={{display:'flex', flexDirection:'column', gap:16}}>
        ...
        <Panel pad={18}>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:8}}>
            {TEMPLATES.filter(t=>t.industries.includes(client.industry)||t.id==='t_custom').map(t =>
              <div key={t.id} onClick={()=>setTemplate(t)} style={{
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:12}}>
              {[
                ...
                <div key={f.l} style={{gridColumn: f.span?'span 2':'auto'}}>
                  ...
                  <input defaultValue={f.v} placeholder={f.ph} style={{
      ...
      <div style={{display:'flex', flexDirection:'column', gap:14, position:'sticky', top:20}}>
        <Panel pad={16}>
          <SectionHeader title="References" eyebrow="Step 4 · pick 3-4"
            action={<span style={{fontSize:11, color:'#F2C97A'}}>{selectedImgs.length} selected</span>}/>
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:6}}>
            {Array.from({length: client.photoCount}, (_,i) => i+1).map(n => {
              ...
              return <div key={n} onClick={()=>setSelectedImgs(sel? selectedImgs.filter(x=>x!==n): [...selectedImgs, n])}
                style={{
                  ...
                  background: `linear-gradient(${n*45}deg, ${client.color}40, ${client.color}15)`,
                  border: `2px solid ${sel ? client.color : 'transparent'}`,
                  position:'relative', overflow:'hidden',
                }}>
                {sel && <div style={{position:'absolute', top:2, right:2, width:14, height:14, borderRadius:7, background: client.color, display:'flex', alignItems:'center', justifyContent:'center'}}>
        ...
        <Panel pad={16}>
          ...
          <div style={{display:'flex', alignItems:'center', gap:10, padding:10, background:VT.bg4, borderRadius:8}}>
            ...
            <div style={{flex:1, minWidth:0}}>
              <div style={{fontSize:12, fontWeight:600, color: VT.text}}>james-vo-reference.mp3</div>
              <div style={{fontSize:10.5, color: VT.textMuted, fontFamily: VT.mono}}>12.4s · @Audio1</div>
            </div>
          </div>
        </Panel>
        ...
        <Panel pad={16}>
          ...
          <div style={{display:'flex', flexDirection:'column', gap:10}}>
            {[
              {l:'Resolution', v:'720p', opts:['480p','720p']},
              {l:'Aspect', v:'9:16', opts:['9:16','1:1','16:9']},
              {l:'Tier', v:'Standard', opts:['Fast','Standard']},
            ].map(r =>
